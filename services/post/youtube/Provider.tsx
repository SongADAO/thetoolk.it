...
export function YoutubeProvider({ children, mode }: Readonly<Props>) {
  ...
  const [accounts, setAccounts, isAccountsLoading] = useUserStorage<
    ServiceAccount[]
  >("thetoolkit-youtube-accounts", [], { initializeWithValue: true });
  ...
  async function exchangeCode(code: string, state: string) {
    try {
      const codeVerifier =
        localStorage.getItem("thetoolkit_youtube_code_verifier") ?? "";
