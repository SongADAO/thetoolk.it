...
interface PinataTokenResponse {
  access_token: string;
}

interface PinataPage {
  id: string;
  name: string;
  access_token: string;
}
...
function formatTokens(tokens: PinataTokenResponse) {
...
function getAuthorizationUrl(
  ...
  return `https://www.pinata.com/v23.0/dialog/oauth?${params.toString()}`;
}
...
async function exchangeCodeForTokens(
  ...
  const response = await fetch(
    "https://graph.pinata.com/v23.0/oauth/access_token",
    {
  ...
  const longLivedTokenResponse = await fetch(
    `https://graph.pinata.com/v23.0/oauth/access_token?${longLivedParams.toString()}`,
  );
...
// Refresh tokens are automatically refreshed by Pinata when any API is called.
async function refreshAccessToken(authorization: OauthAuthorization) {
  ...
  const response = await fetch(
    `https://graph.pinata.com/v23.0/me/accounts?${params.toString()}`,
  );
  ...
  // Refresh tokens are automatically refreshed by Pinata when the API is called.
...
async function getPinataPages(token: string) {
  console.log("Getting Pinata pages...");
  ...
  if (!pagesResponse.ok) {
    ...
    throw new Error(
      `Failed to get Pinata pages: ${error.error?.message ?? pagesResponse.statusText}`,
    );
  }
  ...
  if (!pagesData.data || pagesData.data.length === 0) {
    throw new Error(
      "No Pinata pages found. You need to create a Pinata page to post videos.",
    );
  }

  console.log(`Found ${pagesData.data.length} Pinata page(s)`);

  return pagesData.data;
}

async function getUserInfoFromPage(page: PinataPage): Promise<ServiceAccount> {
  ...
  // Test access to the Pinata account
  const testResponse = await fetch(
    `https://graph.pinata.com/v23.0/${page.id}?${params.toString()}`,
  );

  if (!testResponse.ok) {
    console.error("Pinata API error:", await testResponse.text());
    throw new Error(`Pinata account found but not accessible: ${page.name}:`);
  }

  const testData = await testResponse.json();
  console.log("✅ Pinata Account Details:", testData);
...
async function getAccounts(token: string): Promise<ServiceAccount[]> {
  const facebookPages = await getPinataPages(token);
  ...
  for (const page of facebookPages) {
    try {
      ...
      console.error("Error getting Pinata account:", error);
    }
  }
