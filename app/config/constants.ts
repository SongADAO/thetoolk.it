export const DEBUG_MEDIA = process.env.NEXT_PUBLIC_DEBUG_MEDIA === "true";
export const DEBUG_POST = process.env.NEXT_PUBLIC_DEBUG_POST === "true";
export const DEBUG_STORAGE = process.env.NEXT_PUBLIC_DEBUG_STORAGE === "true";
export const DEBUG_STOP_AFTER_CONVERSION = process.env.NEXT_PUBLIC_DEBUG_STOP_AFTER_CONVERSION === "true";
export const DEBUG_STOP_AFTER_STORAGE = process.env.NEXT_PUBLIC_DEBUG_STOP_AFTER_STORAGE === "true";
export const INFURA_ID = process.env.NEXT_PUBLIC_INFURA_ID;
export const USE_TESTNET = process.env.NEXT_PUBLIC_USE_TESTNET === "true";
export const WALLET_CONNECT_PROJECT_ID: string = process.env.NEXT_PUBLIC_WALLET_CONNECT_PROJECT_ID ?? "";
